...
app.post("/videos", requireCreatorApiKey, async (req,res)=>{
  try{
    ...
    if (!data.title || !data.playbackUrl) return res.status(400).json({ok:false,error:"title and playbackUrl required"});
...
app.post("/videos/:id/comments", async (req,res)=>{
  try{
    ...
    const v = await Video.findByIdAndUpdate(req.params.id, {$push:{comments:{text:t}}},{new:true});
...
function shutdown(sig){
  ...
  server.close(async ()=>{ try{await mongoose.disconnect()}catch{} process.exit(0); });
}
